...
class DailyUpdate extends Model
{
    ...
    public static function createOrUpdate($request, $id = null)
    {
        if ($id)
        {
            self::$daily_update = DailyUpdate::find($id);
        }else{
            self::$daily_update = new DailyUpdate();
        }
        ...
        self::$daily_update->file = $request->file('file') ? fileUpload($request->file('file'), 'updatecontent/', 'daily-update', (isset($id) ? DailyUpdate::find($id)->file : null) ) : $request->file;
