...
class AuthorityContent extends Model
{
    use HasFactory;
    use Searchable;
    ...
    public static function createOrUpdate($request, $id = null)
    {
        if ($id)
        {
            self::$daily_update = AuthorityContent::find($id);
        }else{
            self::$daily_update = new AuthorityContent();
        }
        ...
        self::$daily_update->file = $request->file('file') ? fileUpload($request->file('file'), 'updatecontent/', 'daily-update', (isset($id) ? AuthorityContent::find($id)->file : null) ) : $request->file;
